...
public:
                 ...
                 int keepalive = 60)
    {
        ...
        if (sock_ < 0) { perror("MQTT socket"); return false; }

        // 华为云设备ID含下划线, gethostbyname 不支持, 去掉设备ID前缀解析父域名
        char resolve_host[256];
        const char* dot = strchr(host, '.');
        if (dot && strchr(host, '_') && strchr(host, '_') < dot)
            strncpy(resolve_host, dot + 1, sizeof(resolve_host)-1);
        else
            strncpy(resolve_host, host, sizeof(resolve_host)-1);
        resolve_host[sizeof(resolve_host)-1] = '\0';

        fprintf(stderr, "MQTT: resolving %s\n", resolve_host);
        struct hostent* he = gethostbyname(resolve_host);
        if (!he) { perror("MQTT dns"); close(sock_); sock_ = -1; return false; }
        fprintf(stderr, "MQTT: resolved to %s\n", inet_ntoa(*(struct in_addr*)he->h_addr));
        ...
        {
            perror("MQTT tcp"); close(sock_); sock_ = -1; return false;
        }
        fprintf(stderr, "MQTT: tcp connected\n");
        ...
        fprintf(stderr, "MQTT: sending CONNECT\n");
        if (::send(sock_, buf, pos, 0) < 0)
        {
            perror("MQTT send"); close(sock_); sock_ = -1; return false;
        }

        // 读 CONNACK
        uint8_t resp[4];
        int n = recv(sock_, resp, 4, 0);
        fprintf(stderr, "MQTT: CONNACK n=%d code=%d\n", n, (n>=4)?resp[3]:-1);
        if (n < 4 || resp[0] != 0x20)
        {
            if (n >= 4) fprintf(stderr, "MQTT: CONNACK rejected, return code=%d\n", resp[3]);
